...
CardView<T extends CardProps> (абстрактный)

Назначение: Базовый класс для всех типов карточек товаров
Наследники: BasketItemView, CatalogCardView, ProductPreviewView
Общий функционал:

Отображение заголовка, цены, категории и изображения
Автоматическое применение стилей категорий через categoryMap
Базовая логика рендеринга карточки
FormView<T extends FormProps> (абстрактный)
...
Использует шаблон элемента корзины
BasketView
...
Отображение списка элементов корзины
Показ общей суммы
Управление состоянием кнопки оформления заказа
CatalogCardView

Наследование: CardView<CatalogCardProps>
Назначение: Карточка товара в каталоге
Особенности:

Вся карточка является кликабельной кнопкой
Визуальное отображение состояния "в корзине"
CatalogView

Назначение: Контейнер для отображения каталога товаров
Особенности: Не наследуется от Component, простой контейнер
ContactsView
...
Не может иметь наследников (соответствует требованию)
Может отображать любой контент
...
Карточка каталога (CatalogCardView)

onClick - клик по карточке товара в каталоге
Элемент корзины (BasketItemView)

onDelete - нажатие кнопки удаления из корзины
Корзина (BasketView)

onCheckout - нажатие кнопки оформления заказа
Превью товара (ProductPreviewView)

onToggleCart - нажатие кнопки добавления/удаления из корзины
Форма заказа (OrderView)

onSubmit - отправка формы заказа с данными:

payment - способ оплаты ('card' или 'cash')
address - адрес доставки
Форма контактов (ContactsView)

onSubmit - отправка формы контактов с данными:

email - email пользователя
phone - телефон пользователя
Успешное завершение (SuccessView)

onClose - закрытие окна успешного заказа
Модальное окно (ModalView)
...
Таким образом, презентер — это «дирижёр», который организует работу всех компонентов.

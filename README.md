...
Компонент для управления шапкой сайта, отображает количество товаров в корзине и обрабатывает нажатие на кнопку корзины.

Конструктор:

constructor(container: HTMLElement, events: IEvents)
container (HTMLElement) – корневой элемент шапки (.header), внутри которого ищутся элементы.
events (IEvents) – экземпляр EventEmitter для уведомления о событиях.

Поля класса:

counterEl: HTMLElement – элемент счётчика товаров в корзине.
basketBtn: HTMLElement – кнопка открытия корзины.

Методы:

setCounter(value: number): void – обновляет значение счётчика корзины.

События:

При клике на кнопку корзины эмитится событие:

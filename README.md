...
Методы:
setData(data: IBuyer): void — сохраняет данные покупателя в модели;
getData(): IBuyer — возвращает все данные покупателя;
clear(): void — очищает данные покупателя;
validate(): boolean — проверяет корректность данных (наличие обязательных полей, формат email и телефона).
